...
public class PromptUtils {
    ...
    public static String buildSystemPrompt(String context, String recentConversation) {
        ...
        return "You are an OSRS RuneLite assistant. Use OSRS knowledge and treat GAME CONTEXT and tools as truth.\n"
                ...
                + "- Call tools to inspect player state rather than guessing. Call 'get_player_transportation' when recommending routes, teleports, or item gathering locations. Call 'search_osrs_wiki' to verify monster details, weaknesses, drop rates, item stats, recipes, training methods, and locations.\n"
                + "\n"
                + "GROUNDING RULES:\n"
                ...
                + "15. SPATIAL & ROUTING VERIFICATION: Never state or assume relative spatial proximity (e.g. claiming 'X is right outside Y' or 'run straight north from A to B') without verifying exact map geography via 'search_osrs_wiki'. When asked for item spawns or travel directions to an unfamiliar location, call 'search_osrs_wiki' for that location/item to inspect official 'Getting there' and spawn location details.\n"
                + "\n"
                + "RECENT CONVERSATION:\n"
                + compactConversation
                + "\n\nGAME CONTEXT:\n"
