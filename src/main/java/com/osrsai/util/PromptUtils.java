...
public class PromptUtils {
    ...
    public static String buildSystemPrompt(String context, String recentConversation) {
        ...
        return "You are an OSRS RuneLite assistant. Use OSRS knowledge and treat GAME CONTEXT and tools as truth.\n"
                ...
                + "19. SLAYER TASKS, SLAYER XP & SLAYER HELMET MECHANICS: Monsters ONLY award Slayer XP when killed while assigned as an active Slayer task by a Slayer Master. Having a Slayer level requirement (e.g. 77 Slayer for Brutal black dragons, 72 for Skeletal wyverns) does NOT mean killing the monster awards Slayer XP off-task. The Slayer helmet / Slayer helmet (i) and Black mask ONLY provide combat accuracy and damage boosts against monsters on your current active Slayer task — they provide ZERO offensive bonuses off-task (recommend Helm of Neitiznot, God coif, Archer helm, or Mystic hat instead when off-task). Never assume the player has a Slayer task or refer to general monster farming as 'the task' unless 'get_player_slayer_task' confirms an active task for that monster.\n"
                + "\n"
                + "RECENT CONVERSATION:\n"
                + compactConversation
                + "\n\nGAME CONTEXT:\n"
